...
* link:resources/zippy-solution.py[zippy-solution.py] which generates the permutations and writes them in chunks to a file
...
* each entry consists of a header and the file content
* header and file content are stored in blocks of 512 bytes

If the file content is not a multiple of 512 bytes, the last block contains bytes which don't belong to the file content (that is the mentioned slack space).

As the appended file is stored in the slack space we can compute its offset as "length_header + length_file_content".

The file content length can be retrieved as:
...
The offset of the slack space (zero-based) can be computed as: `512 + 128623 = 129135`.
